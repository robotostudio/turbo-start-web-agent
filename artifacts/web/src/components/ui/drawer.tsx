...
/* LOCAL DIVERGENCE from the vendored Base UI source: the scrim is
   `bg-foreground/10`, not the upstream `bg-black/10`. This site is dark, so a
   10% black veil over a near-black page is invisible and the nav panel floats
   with no separation from the content behind it. `--foreground` inverts with
   the theme, which is the same intent expressed in a token. Re-pulling this
   component from upstream will reintroduce the literal — keep this change. */
