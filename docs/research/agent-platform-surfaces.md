...
- Instructions: v0 "Instructions" are **account-level saved prompts applied per-chat from the UI** (v0.app/docs/instructions) — presets "Be Concise" / "Plan Mode" plus custom ones. Project settings hold env vars/integrations/GitHub/domains, not an instructions file.
- **CORRECTED 2026-08-20 by direct test — v0 DOES read repo instruction files.** This section previously concluded "no AGENTS.md support documented," which was an accurate reading of the docs and a wrong conclusion about the product. Asked what its rules were, a v0 chat on an imported repo answered: *"The repo's main change rules come from the root `AGENTS.md`, which is loaded as repository-specific instructions,"* and separately cited `.agents/skills/`, naming `compose-page` and `sync-changes`. It then followed them, including the commit-trailer convention, swapping in `Agent: v0` unprompted.
  The docs are **silent**, not negative — v0's own pages neither document nor deny a repo-file surface, and the only Vercel page that mentions AGENTS.md is the **Vercel Agent code-review** changelog, a different product. Treat "not documented" as "untested" here rather than "unsupported"; this entry is the reason that distinction is worth writing down.
- **Also observed, not in any doc:** the working branch is named from the chat title (e.g. `repository-contribution-guidelines`), not the `v0/*` pattern the branch docs describe; and v0 commits under its own identity (`v0 <it+v0agent@vercel.com>`) rather than the connecting user's.
